...
using AsukaBot_2._0.Module.RPGModule.Classes;
using AsukaBot_2._0.Module.RPGModule.Logic;
using AsukaBot_2._0.Module.RPGModule.Logic.Guilds;
using AsukaBot_2._0.Module.RPGModule.Logic.Items;
using AsukaBot_2._0.Module.RPGModule.Logic.Questing;
using Discord;
...
namespace AsukaBot_2._0.Module.RPGModule
{
    public class RPGMain : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task ItemInfo(string para1, string para2 = null, string para3 = null, string para4 = null)
        {
            ...
            for (int i = 0; i < CheckUpList.GetAllItemsList().Count; i++)
            {
                if (temp.ToLower() == CheckUpList.GetAllItemsList()[i].Getname().ToLower())
                {
                    ...
                    try
                    {
                        if (CheckUpList.GetAllItemsList()[i] is WeaponItem)
                        {
                            Weapon = (WeaponItem)CheckUpList.GetAllItemsList()[i];
            ...
            if (doesitemExsits)
            {
                ...
                else
                {
                    if (AllPlayers[user].GetInventory().DoesTimeExistsInInventory(temp))
                    {
                        if (AllPlayers[user].GetInventory().DoesTimeExistsInInventory(temp))
                        {
                            if (AllPlayers[user].GetInventory().GetinventoryItemByName(temp) is WeaponItem || AllPlayers[user].GetInventory().GetinventoryItemByName(temp) is MagicAttacks)
                            {
        ...
        public async Task StatsDisplay()
        {
            ...
            else
            {
                ...
                builder.AddField("Class", AllPlayers[temp].GetClass().GetClassName());
                builder.AddField("Power", Statsholder.GetPower().GetPowerLvl());
                builder.AddField("Magic", Statsholder.GetMagic().GetMagicLvl());
                builder.AddField("Dexterity", Statsholder.GetDexterity().GetDexterityLvl());
                builder.AddField("Intelligence", Statsholder.GetIntellegence().GetIntellegencLvl());
                builder.AddField("Vitallity", Statsholder.GetVitallity().GetVitallityLvl());
                builder.AddField("Luck", Statsholder.GetLuck().GetLuckLvl());
